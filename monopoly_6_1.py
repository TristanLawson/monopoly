...
class Player(object):               #each player has their own Player object
    ...
    def addTransport(self):             #increment number of transports owned
        if self.numTransports < 4:          #check if valid
            self.numTransports += 1         #increment
        else:
            raise PropertyIssue('You already own all the transports')
        
    def removeTransport(self):          #decrement number of transports owned
        if self.numTransports > 0:          #check if valid
            self.numTransports -= 1         #decrement
        else:
            raise PropertyIssue('You do not own any transports')
    ...
    def getProperties(self):                    #return Player.properties as list of 1's and 0's
        return self.properties
    
    def getNumTransports(self):                 #return number of transports owned
        return self.numTransports
    ...
    def getNetWorth(self):                      #calculate and return net worth as int
        ...
        worth += self.numTransports*trans[1].getCost()  #add (#trans owned)*(cost of a transport)
        return int(worth)
...
class Property(object):                     #all 22 properties are Property objects
    ...
    def getRentCost(self):                  #determine and return rent cost as int
        if self.mortgaged:                      #if mortgaged, rent is 0
            ...
            return self.rent[self.val]     #return correct index of rent[] according to self.val
...
class Transport(object):        #transport capital
    
    def __init__(self,num,name):    #num is an int, name is a str
        self.num = num
        self.name = name
        self.owner = 0
        self.cost = 100
        self.fees = [0,50,100,150,200]
    
    #CHANGE ATTRIBUTES
    def setOwner(self,pn):          #declare the owner of the transport (as an int)
        if pn in range(len(pi)):        #ensure valid player
            self.owner = pn
        else:
            raise InputError('invalid player')
    
    #INFO
    def getNum(self):
        return int(self.num)             #return Transport.num as int
    
    def getName(self):
        return str(self.name)                #return Transport.name as str
    
    def getOwnerNum(self):
        if self.owner in range(len(pi)):
            return int(self.owner)                  #0 if Bank, pos int if player
        else:
            raise PropertyIssue('Transport #'+str(self.num)+' has invalid owner')
    
    def getOwner(self):                     #return Player that owns transport
        if self.owner == 0:                     #raise exception if owned by bank
            raise PropertyIssue('Transport is owned by the bank')
        else:                                   #otherwise return Player
            return pi[self.getOwnerNum()]       #by calling getOwnerNum()
    ...
    def getCost(self):                      #return Transport.cost as int
        return int(self.cost)

    def getFee(self):           #return cost of using transport
        p1 = self.getOwner()
...
def numToProp(num):         #given number, return associated Property
    ...
    #private
def allToTrans(anyForm):       #given int/transport, return associated Transport
    if type(anyForm) == int:
        return numToTrans(anyForm)
    elif isinstance(anyForm,Transport):
        return anyForm
    else:
        raise InputError(str(anyForm)+' is not a valid transport')
...
def initProperties(): #num  name                    cost  hc  rent
    ...
    #private            init all 4 transports
def initTransports():   #num name
    trans[1] = Transport(1, 'Monopoly Rail')
    trans[2] = Transport(2, 'Monopoly Air')
    trans[3] = Transport(3, 'Monopoly Cruise')
    trans[4] = Transport(4, 'Monopoly Space')
...
def payFee(pin,trin):
    p1 = allToPlayer(pin)
    trans = allToTrans(trin)
    if trans.getOwnerNum() == 0:
        raise PropertyIssue('No one owns this transport')
    else:
        payPlayer(p1,trans.getFee(),trans.getOwner())
...
def removePlayer(pin):          #remove player from game
    ...
    for i in range(num+1,len(pi)):    #decrement all higher player numbers
        ...
        pi[i-1] = pi[i]
        pi[i] = 0
    shiftAssets(num)                #adjust player numbers for property & transports
    pi.remove(0)                    #remove 0 from pi
    print(p1.getName(),'has been removed from the game')
    
    #private
def shiftAssets(num):           #for removing a player, change all their assets to the bank
                                #and adjust the owner number for all other assets accordingly
    for p in range(1,len(prop)):
        ownerNum = prop[p].getOwnerNum()    #properties
        if ownerNum == num:                     #if owned by removed player
            changePropOwner(0,prop[p])          #change ownership to the bank
        elif ownerNum > num:                    #if owner's number changes               
            prop[p].setOwner(ownerNum-1)        #subtract owner number by 1
    for t in range(1,len(trans)):
        ownerNum = trans[t].getOwnerNum()   #transports
        if ownerNum == num:                     #do the same thing
            trans[t].setOwner(0)
        elif ownerNum > num:
            trans[t].setOwner(ownerNum-1)
